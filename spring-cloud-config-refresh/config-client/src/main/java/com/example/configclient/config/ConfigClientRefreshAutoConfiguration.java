...
public class ConfigClientRefreshAutoConfiguration implements SchedulingConfigurer {
    ...
    public void configureTasks(ScheduledTaskRegistrar scheduledTaskRegistrar) {
        long refreshIntervalInMilliseconds = getRefreshIntervalInMilliseconds();
        logger.info(String.format("Scheduling config refresh task with %s second delay", refreshInterval));
        scheduledTaskRegistrar.addFixedDelayTask(new IntervalTask(() -> refreshEndpoint.refresh(), refreshIntervalInMilliseconds, refreshIntervalInMilliseconds));
    }
