...
​		集中式的配置文件不适合分布式环境，由此引来配置中心，配置中心有很多种，选择其中做对比

​	spring-cloud-config    ，netflix archaius ，ctirp apollo，disconf

​	

| 对比性            | 重要性 | spring-cloud-config   | netflix archaius | ctirp apollo                   | disconf                                |
| ----------------- | ------ | --------------------- | ---------------- | ------------------------------ | -------------------------------------- |
| 功能特性          |        |                       |                  |                                |                                        |
| 静态配置管理      | 高     | 基于 file             | 无               | 支持                           | 支持                                   |
| 动态配置管理      | 高     | 支持                  | 支持             | 支持                           | 支持                                   |
| 统一管理          | 高     | 无，需要git、数据库等 | 无               | 支持                           | 支持                                   |
| 多维度管理        | 高     | 无，需要git、数据库等 | 无               | 支持                           | 支持                                   |
...
| 配置定时拉取      | 高     | 无                    | 无               | 支持                           | 依赖事件驱动client重启或server推送     |
| 用户权限管理      | 中     | 无，需要git、数据库等 | 无               | 支持                           | 支持                                   |
| 授权、审核、审计  | 中     | 无，需要git、数据库等 | 无               | 界面直接提供发布历史和回滚按钮 | 操作记录落数数据库，但是无查询接口     |
| 配置版本管理      | 高     | git                   | 无               | 支持                           | 操作记录落数数据库，但是无查询接口     |
| 配置合规检查      | 高     | 不支持                | 不支持           | 支持（还需完善）               |                                        |
| 实例配置监控      | 高     | 需要spring admin      | 不支持           | 支持                           | 支持，可以查看每个配置在那台机器上加载 |
| 灰度发布          | 中     | 不支持                | 不支持           | 支持                           | 不支持部分更新                         |
| 告警通知          | 中     | 不支持                | 不支持           | 支持邮件式告警                 | 支持邮件式告警                         |
| 统计报丧          | 中     | 不支持                | 不支持           | 不支持                         | 不支持                                 |
| 依赖关系          | 高     | 不支持                | 不支持           | 不支持                         | 不支持                                 |
| 技术路线          |        |                       |                  |                                |                                        |
| 支持spring boot   | 高     | 原生支持              | 低               | 支持                           | 与spring boot 无关                     |
| 支持spring config | 高     | 原生支持              | 低               | 支持                           | 与spring boot 无关                     |
| 客户端支持        | 低     | java                  | java             | java .net                      | java                                   |
| 业务系统入侵性    | 高     | 入侵性弱              | 入侵性弱         | 入侵性弱                       | 入侵性弱 支持注解 xml                  |
| 可依赖组建        | 高     |                       |                  |                                |                                        |
| 可用性            |        |                       |                  |                                |                                        |
| 单点故障（SPOF）  | 高     | 支持HA部署            | 支持HA部署       | 支持HA部署                     | 支持HA部署，zk提供高可用               |
| 多数据中心部署    | 高     | 支持                  | 支持             | 支持                           | 支持                                   |
| 配置获取性能      | 高     | unknow                | unknow           | unknow                         | unknow                                 |
| 易用性            |        |                       |                  |                                |                                        |
| 配置界面          | 中     | 无，需要git、数据库等 | 无               | 统一界面（ng编写）             | 统一界面                               |
|                   |        |                       |                  |                                |                                        |
...
​	还是需要刷新命令  http://localhost:9090/actuator/bus-refresh post 当然可以将此配置到 推代码上默认配置。

​	可以在刚才的命令后面拼上?destination=** 刷新所有的客户端，码云上的webHook 对应config server 的回调地址
...
​			注意：health检查 走的是域名（不是一个机器的话） 需要ip 映射

### 	 2.consul深入

​		**模块**：

​		spring-cloud-consul-binder: 对consul **事件功能封装**

​		spring-cloud-consul-config ：对consu**l配置功**能封装

​		spring-cloud-consul-core: **基础配置和健康检查模块**

​		spring-cloud-consul-discovery: 对consul**服务治理功能**封装

​		注意：consul的时间功能比较弱化，应用比较多的是**服务治理和配置功能**

​		**spring cloud consul discovery**

​		在功能上分成两部分：**服务注册**（服务上线，服务下线）和**服务发现**（心跳报告《默认不开启》 服务发现）

​		

| 配置项  | 修改方法 前缀 spring.cloud.consul.discovery | 适用场景                                                     |
| ------- | ------------------------------------------- | ------------------------------------------------------------ |
| address | prefer-ip-address=true                      | 注册的是ip而非主机名                                         |
| address | ip-address=${HOST_IP}                       | 如果服务部署在docker容器中，需要告诉consul真实的地址         |
| Port    | port=${HOST_PORT}                           | 如果服务部署在docker容器中，需要告诉consul真实的端口         |
| Tags    | tags=${GROUP},test                          | 如果需要给相同服务进行分组，可以使用该配置给服务打不同的标签，不同的标签之间用,隔开 |
| Check   | health-check-interval=20s                   | consul 默认对服务的健康检查时间是10s                         |
| Check   | health-check-path=/health.json              | 默认的健康检查路径是/actuator/health。如果你没有使用spring-cloud-actuator，或者由于其他原因想定制健康检查路径，可以修改此配置 |

​	**服务发现**

​	 服务发现功能有两种 **ribbon** 和**DiscoveryClient**

​	**Feign** 或者 @**LoadBalancerd**  使用的是**ConsulServerList**

​	客户端想独立使用服务发现的话，可以直接使用 **DiscoveryClient**

​	使用 tag分组

​	**注意**：**spring cloud consul config配置刷新原理** ：配置并没有实时生效，而是定时任务去拉取最新配置，若发现变更（通过consulIndex 版本对比）通过SpirngEventPublisher重新刷新配置

​			**spring cloud consul config 只支持k/v 那么可以将 k存应用名 v存所有配置（将yml 、properties存在V）**
...
​			请求-->网关处sso认证鉴权-->redis/mysql->不同模块请求

#### 		4.客户端Token与网关集合

​			请求+token->网关认证鉴权->返回token+请求转发

​			token不适合存储大量数据，可以使用第三方redis/mysql 存储这些信息，token的注销要在网关
...
​			外部请求->网关认证生成token 放到header->服务调用 header 之间调用

​			内部请求->网关拦截生成信息到header
...
​	Skywalking 概述用于提供分布式追踪功能，从5.x开始项目成为一个完成功能的APM系统，被用于追踪、监控和诊断分

布式系统，特别是使用微服务架构、云原生或者容积技术。
...
​	 跨语言的rpc框架 类似于thrift，双向流，流控，头部压缩，单个tcp连接上多复用等

### 3.使用

​	grpc-spring-boot-starter 开源项目
...
​		1.蓝绿发布   蓝绿（表示旧版本）两套版本，将流量从绿切换到蓝 完成过渡

​		2.滚动发布  一般将集群中的少量节点进行更新后上线，陆续更新部分节点，直到全部成为新版本

​		3.灰度发布   

​				**A/B**    A为线上版本 B为迭代版本  将一部分流量切换给 B 收集用户反馈，逐步改进B 直到用户完全可以接受 ，							则替换，有测试的过程（与蓝绿的区别）

​				**金丝雀**   在应用集群中部署一台实例作为 金丝雀 引一部分流量 手机问题 及时调整 达到上线标准 替换集群中其								他实例

​		![1565143406086](.\assets\1565143406086.png)
...
​		2.一致性，事务提交 \ 回滚 保持数据的一致性 一致性不会被破坏

​		3.隔离性，事务与事务之间的独立性，读未提交、读已提交、可重复度、串行化

​		4.持久性，数据修改时持久化的

​	**X/Open DTP 模型**

​		当一个业务操作涉及多个数据源的操作，那么院线单一的数据事务（本地事务）不能满足需求，为此

​	X/Open 组织定义了 DTP模型，规范全局事务处理
...
​		是X/Open 在 2pc的基础上给事务管理器TM与资源管理器RM之间通信定义的接口，遵循java规范
...
​		atomikos    bitronix  arayana

​	**2.tcc**

​		蚂蚁金服的实现

![1565172111492](.\assets\1565172111492.png)

​	**3.SAGA模式**

![1565172243040](.\assets\1565172243040.png)
